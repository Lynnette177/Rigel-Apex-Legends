...
namespace OW {
	namespace Render {
		...
		inline void DrawSeerLikeHealth(float x, float y, int shield, int max_shield, /*int armorType,*/ int health) {
			int bg_offset = 3;
			int bar_width = 158;
			// 4steps...2*3=6
			// 38*4=152 152+6 = 158
...
*/
		...
		void DrawHealthBar(Vector2 screenPos, float height, float currentHealth, float maxHealth,int MaxShield,int nowshield) {
			...
			if (MaxShield == 125) HealthColor = Color(255, 0, 0);
			else if (MaxShield == 100) HealthColor = Color(204, 0, 204);
			else if (MaxShield == 75) HealthColor = Color(153, 102, 255);
			else if (MaxShield == 50) HealthColor = Color(255, 228, 225);
			if (nowshield == 0) HealthColor = Color(10, 255, 10);
			if (currentHealth <= (80)) {
